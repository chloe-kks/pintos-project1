...
struct thread
{
	...
	int original_priority;				/* The priority that does not changes even during priority donation.*/
	int64_t wakeup_ticks;				/* The ticks required for the thread to wakeup. */
	bool while_;						/* Used to check whether to go inside the while loop of lock_acquire. Used in synch.c */
	
										/* Shared between thread.c and synch.c. */
	struct list_elem elem;              /* List element. */

	fp_t recent_cpu;						/* The estimate of the CPU time the thread has used recently. */
